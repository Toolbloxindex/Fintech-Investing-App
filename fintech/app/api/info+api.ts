...
export async function GET(request: Request) {
  ...
  if (!ids) {
    return Response.json(
      { error: "You must provide at least one 'ids' query parameter (e.g. ?ids=1,1027)" },
      { status: 400 }
    );
  }

  // Build the API URL with the 'id' parameter
  const apiUrl = `https://pro-api.coinmarketcap.com/v2/cryptocurrency/info?id=${ids}`;
...
const data = {
  ...
  "5426": {
    ...
    "52": {
        ...
        "symbol": "XRP",
        "category": "coin",
        "description": "XRP (XRP) is a cryptocurrency . XRP has a current supply of 99,985,901,574 with 59,239,646,627 in circulation. The last known price of XRP is 3.10278177 USD and is up 0.39 over the last 24 hours. It is currently trading on 1666 active market(s) with $6,047,291,091.58 traded over the last 24 hours. More information can be found at https://xrpl.org/.",
        "slug": "xrp",
